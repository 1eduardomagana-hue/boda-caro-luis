...
import { COLORS, SYSTEM_PROMPT } from '../lib/constants.js'
...
export default function Importar({ project }) {
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages:[{role:'user',content:`Analiza esta nota de planeación de boda:\n\n${text}`}]
        })
      })
  ...
  const aprobar = async () => {
    ...
    setSaving(false); setDone(true); setStep('done')
  }

  const reset = () => { setStep('input'); setText(''); setResultado(null); setError(''); setDone(false); setProvs([]); setEvts([]); setTasks([]) }
  ...
  if (step==='analyzing') return (
    <div><PageHeader title="Importar con IA" />
      <Card><div style={{display:'flex',flexDirection:'column',alignItems:'center',padding:'60px 20px',gap:14}}>
        ...
        <div style={{fontSize:12,color:C.textLight,textAlign:'center',lineHeight:1.6}}>Extrayendo proveedores, eventos, tareas y pagos</div>
      </div></Card>
    </div>
  )
        ...
        <Card style={{borderTop:`3px solid ${C.champagne}`}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:12}}>
            <div>
              ...
              <span style={{display:'inline-flex',alignItems:'center',gap:4,background:`${resultado.confianza>=.8?C.green:resultado.confianza>=.5?C.amber:C.red}22`,color:resultado.confianza>=.8?C.green:resultado.confianza>=.5?C.amber:C.red,borderRadius:99,padding:'2px 10px',fontSize:11,fontWeight:600}}>
  ...
  return (
    <div>
      <PageHeader title="Importar con IA" subtitle="Pega una nota de Apple Notes y la IA extrae y clasifica todo" />
      <div className="grid-2" style={{alignItems:'start'}}>
        <div className="section-gap">
          <Card>
            ...
            {text.length>0 && <div style={{fontSize:11,color:C.textLight,marginTop:6,textAlign:'right'}}>{text.length.toLocaleString()} caracteres</div>}
          </Card>
          {error && <div style={{background:'#FAEAEA',border:`1px solid #E0B0B0`,borderRadius:8,padding:'10px 14px',fontSize:12.5,color:C.red}}>{error}</div>}
          <Btn onClick={analizar} disabled={!text.trim()} fullWidth style={{padding:'14px',fontSize:14}}>✦ Analizar con IA</Btn>
        </div>
        <div className="section-gap">
          <Card>
            ...
            {[['01','Pega tu nota','Copia el contenido de Apple Notes tal como está.'],
              ...
              <div key={n} style={{display:'flex',gap:14,padding:'10px 0',borderBottom:`1px solid ${C.beige}`}}>
                ...
                <div><div style={{fontSize:13,fontWeight:500,color:C.darkTaupe,marginBottom:2}}>{t}</div><div style={{fontSize:12,color:C.textLight,lineHeight:1.5}}>{d}</div></div>
              </div>
            ))}
          </Card>
          <Card>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              {[['💳','Proveedores y pagos'],['📅','Fechas y eventos'],['✓','Tareas y checklists'],['👥','Invitados'],['🗓️','Plan horario'],['🔗','Links'],['⚠️','Duplicados potenciales'],['🏷️','Categoría sugerida']].map(([ic,l])=>(
                <div key={l} style={{display:'flex',gap:8,alignItems:'center',background:C.cream,borderRadius:7,padding:'8px 10px'}}>
                  <span style={{fontSize:14}}>{ic}</span><span style={{fontSize:12,color:C.darkTaupe}}>{l}</span>
                </div>
              ))}
            </div>
          </Card>
